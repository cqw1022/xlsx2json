...
var arraySeparator;
var arrayArraySeparator;
var objectArraySeparator;
...
module.exports = {
   ...
   */
  toJson: function (src_excel_file, dest_dir, head, separator, arrayArraySeparatorIn, objectArraySeparatorIn) {

    arraySeparator = separator;
    arrayArraySeparator = arrayArraySeparatorIn;
    objectArraySeparator = objectArraySeparatorIn;
  ...
  if (sheet.data && sheet.data.length > 0) {
    ...
    for (var i_row = head; i_row < sheet.maxRow; i_row++) {
      ...
      for (var i_col = 0; i_col < col_type.length; i_col++) {
        ...
        case '[[]]': //[number] [boolean] [string]  todo:support [date] type
          if (cell && cell.value && cell.value.trim().length) {
            parseArrayArrayField(json_obj, col_name[i_col], cell.value);
          } else {
            json_obj[col_name[i_col]] = [];
          }
          break;
...
function parseArrayArrayField(row, key, value) {
  
  var obj_array = [];

  if (value) {
    if (value.indexOf(arrayArraySeparator[0]) !== -1) {
      obj_array = value.split(arrayArraySeparator[0]);
    } else {
      // obj_array.push(value.toString());
      if (isNumber(value)) {
        obj_array.push(Number(value));
      } else if (isBoolean(value)) {
        obj_array.push(toBoolean(value));
      } else if (isDateType(value)) {
        obj_array.push(convert2Date(value));
      } else {
        obj_array.push(value.toString());
      }
    }
  ...
  obj_array.forEach(function (e) {
    if (e) {
      if (e.indexOf(arrayArraySeparator[1]) !== -1) {
        result.push(e.split(arrayArraySeparator[1]));
      } else {
        if (isNumber(e)) {
          result.push(Number(e));
        } else if (isBoolean(e)) {
          result.push(toBoolean(e));
        } else if (isDateType(e)) {
          result.push(convert2Date(e));
        } else {
...
function parseObjectArrayField(row, key, value) {
  ...
  if (value) {
    if (value.indexOf(objectArraySeparator[0]) !== -1) {
      obj_array = value.split(objectArraySeparator[0]);
    } else {
      obj_array.push(value.toString());
    }
  }
  ...
  obj_array.forEach(function (e) {
    if (e) {
      result.push(array2object(e.split(objectArraySeparator[1])));
    }
  });
